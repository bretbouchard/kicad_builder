...
def test_apa102_footprint():
    """Validate APA102-2020 footprint against datasheet specifications"""
    mod = KicadMod.from_file(
        "inbox/APA-102-2020-256-8/LED_APA-102-2020-256-8.kicad_mod"
    )
    ...
    assert 2.4 <= mod.size[0] <= 2.6, (
        f"Courtyard X {mod.size[0]}mm invalid (expected 2.5±0.1mm)"
    )
    # Original Y: 2.0±0.1mm + 0.25 clearance per side = 2.5±0.1mm total
    assert 2.4 <= mod.size[1] <= 2.6, (
        f"Courtyard Y {mod.size[1]}mm invalid (expected 2.5±0.1mm)"
    )
    ...
    data_pads = [
        p for p in mod.pads 
        if any(n in p.name for n in ["2_1", "2_2"])
    ]
    ...
    for p in mod.pads:
        ...
        for op in adjacent:
            ...
            assert distance >= 0.2, (
                f"{p.name}-{op.name}: {distance:.2f}mm < 0.2mm"
            )
    
    # Manufacturing specs
    paste_layers_present = any(
        "F.Paste" in p.layers 
        for p in mod.pads
    )
    ...
    assert 0.24 <= mod.courtyard_clearance <= 0.26, (
        f"Courtyard clearance {mod.courtyard_clearance:.3f}mm invalid "
        f"(should be {target_clearance}±0.01mm)"
    )
